...
import { setCustomProviders, type CustomProviderSpec } from "./model/custom-providers.ts";
...
import { createHarnessRouter, resolveRuntimeChoiceDurable, type HarnessModelIds } from "./harness/harness-router.ts";
...
export interface BuildAppOverrides {
  ...
  harnesses?: readonly Harness[] | ((dependencies: HarnessFactoryDependencies) => readonly Harness[]);
  defaultHarnesses?: (dependencies: HarnessFactoryDependencies) => readonly Harness[];
  replaceHarnesses?: boolean;
  harnessModelIds?: HarnessModelIds;
}

export interface HarnessFactoryDependencies {
  config: Config;
  signals: RunSignalStore;
  tasks: TaskStore;
  mcpTools: () => ReturnType<McpToolService["toolDefs"]>;
  resolveBaseModelId: () => string | undefined;
  resolveProviderKeys: () => Promise<Record<string, string>>;
...
export function buildApp(config: Config, overrides: BuildAppOverrides = {}): BuiltApp {
  ...
  const resolveCustomHarnessProviders = async (): Promise<Array<{ spec: CustomProviderSpec; apiKey?: string }>> => {
  ...
  const harnessDependencies: HarnessFactoryDependencies = {
    config,
    signals: runSignals,
    tasks,
    mcpTools,
    ...
    resolveCustomProviders: resolveCustomHarnessProviders,
  };
  if (!overrides.replaceHarnesses) {
    const defaultHarnesses = overrides.defaultHarnesses?.(harnessDependencies) ?? [createMockHarness()];
    for (const adapter of defaultHarnesses) adapters.set(adapter.profile.id as HarnessId, adapter);
  }
  const injectedHarnesses =
    typeof overrides.harnesses === "function" ? overrides.harnesses(harnessDependencies) : (overrides.harnesses ?? []);
  for (const adapter of injectedHarnesses) adapters.set(adapter.profile.id as HarnessId, adapter);
  const availableHarnesses = [...adapters.keys()];
  const harnessModelIds = overrides.harnessModelIds ?? {};
  const fallbackHarness = config.harness as HarnessId;
  const fallbackAdapter = adapters.get(fallbackHarness);
  if (!fallbackAdapter) {
    throw new Error(`HARNESS=${fallbackHarness} requires an injected ${fallbackHarness} harness adapter`);
  }
  const fallback = {
  ...
  const harness = createHarnessRouter(adapters, fallbackAdapter, (input) =>
...
export function serverDeps(
  ...
  return {
    ...
    ...(config.brandingDefault ? { brandingDefault: config.brandingDefault } : {}),
    harnessId: config.harness,
    harnessIds: built.availableHarnesses,
    harnessModelIds: built.harnessModelIds,
