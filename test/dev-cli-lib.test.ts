...
test("OpenCode config is strict, pinned, and inherits the Pi model", () => {
  ...
  assert.throws(() => loadConfig({ HARNESS: "bogus" }), /use mock, pi, opencode, codex, claude, or alego/);
  assert.throws(() => loadConfig({ HARNESS: "PI" }), /use mock, pi, opencode, codex, claude, or alego/);
});
